...
| `prototype.md` | safe read-only source for production-shaped data, worktree/branch convention for disposable prototype branches, backend diagram convention, frontend review mode | prototype (hand-invoked, not part of implement) | `factory-prototype/templates/prototype.md` + `factory-prototype/onboarding.md` |
...
| `initiatives.md` | registry of active initiative-listener agents — persistent, per-channel conversational agents, one row per slug/channel/scope/deploy-behavior | initiative-start, initiative-listen, listener | `factory-initiative-start/templates/initiatives.md` + that skill. Per-initiative identity docs (`initiatives/<slug>.md`) aren't a registry row — `factory-initiative-start` creates one per initiative live, not as part of this interview. |
