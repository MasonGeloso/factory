...
1. **Understand needs.** For new or ambiguous work, pin down the output format, fidelity, option count, constraints, and the design systems / UI kits / brands in play by reading the task/plan/issue and the codebase — not by asking the user. Make the most defensible call on anything still open and state it as an assumption rather than blocking on it. (See chapter 3.)
...
# 3. Assumptions, not question rounds

Bad designs come from missing context, not from missing skill — but in this environment there is no
user standing by to answer a kickoff question round. You run inside an autonomous pipeline
(`factory-implement`, `factory-prototype`): the task/issue, its comments, the plan doc, and the
codebase are the brief. Extract everything from them before inventing anything.

**Always resolve from context first:**
- The starting point and product context — UI kit, design system, codebase, screenshots referenced by
  the task. Read them; do not wait to be handed them.
- Whether variations are wanted, and on which axes (overall flow, individual screens, specific
  components, color, typography, copy, motion) — the task/plan usually says, or the surface implies it.
- Whether the brief wants options that match existing patterns, novel/creative ideas, or a mix.
- What tweaks (if any) should be exposed in the final design.
- The audience, format, length, and tone of the output.

**When something is genuinely unresolvable from context** (no design system in the repo, no brand
reference anywhere, no signal either way in the task), do not block on it — commit to the most
defensible choice per chapter 6's defaults, and state it plainly as an assumption in your summary (and,
inside `factory-implement`, post it on the issue per that skill's assumption-logging rule). A stated
assumption a reviewer can correct beats a stalled run.

**Reserve an actual question for the user** only for the narrow cases this pipeline already allows
asking in (see the E-level table in `factory-implement`, and the review step at the end of
`factory-prototype`'s frontend track) — never as a default first move.
...
If you genuinely cannot find context anywhere — the codebase, the task/plan, any attached
screenshots — **do not invent a brand or visual language out of thin air by default.** Invoke
`frontend-aesthetic-direction` to commit to a deliberate aesthetic instead, and say plainly in your
summary that no existing design system was found.
...
## Don't add scope unrequested

If you think additional sections, copy, or content would improve the design, **do not unilaterally add
it.** Build what the task actually asked for, note the idea as an open suggestion in your summary (or
on the issue, per `factory-implement`'s assumption-logging rule), and let the user or a review gate
...
## Derive the persona from the brief, then state it

For new work, resolve from the task/plan/codebase rather than asking:
...
If the brief genuinely leaves one of these open, pick the most defensible answer and say so in your
summary — don't leave the persona implicit and don't stall waiting for it to be specified.
...
If you think additional sections, pages, copy, or content would improve the design, **do not add it
unrequested.** Adding scope is a decision that belongs to the task's owner, not to you — flag the idea
instead of building it (see chapter 5, "Don't add scope unrequested").
...
You have the following procedures available, each a phased checklist with explicit checks and fixes,
each in its own file under this skill's `skills/` directory (`skills/factory-frontend/skills/<name>.md`
from the repo root). They are plain reference docs, not separately Skill-tool-invocable skills — when
one applies, read its file and follow its phases inline, in this same turn. Each description states its
trigger. When in doubt about whether a review procedure applies, run it: a redundant check is cheap, an
unreviewed deliverable is not.
...
Skills can be chained. A typical greenfield flow: `frontend-aesthetic-direction` → `wireframe` → `make-a-prototype` → `polish-pass`. Or for a brand-aware flow: `design-system-extract` → `generate-variations` → `make-tweakable` → `polish-pass`.

# Final principle

Designs that look intentional come from thinking that is intentional. Every choice has a reason. Every element earns its place. Every interaction gives feedback. Every detail is polished or honestly placeholder'd. The user is your manager — show your work, state your assumptions, and deliver less but better.
