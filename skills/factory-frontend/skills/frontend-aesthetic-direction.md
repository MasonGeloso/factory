...
Double-check: no brand guide, no existing app or product to match, no reference site named in the task, no partial design system in the codebase. If any exist, **stop and use them** — aesthetic direction is for true greenfield.

## Phase 2: Discover the intent

Derive from the task, plan, and codebase (or take as stated if the brief already says it): **three
adjectives** for the desired feel; **audience**; **industry context**; **reference designs** the brief
points at (and what specifically — the type, spacing, color, tone, density?); **off-limits** aesthetics
or tropes.

If the brief leaves this open, propose **4 distinct visual directions**, each specified concretely — background hex / accent hex / display + body typeface, with a one-line rationale tied to the brief — and commit to the strongest fit yourself, stating the rationale and the other three in your summary so a reviewer can redirect you. The directions must not share a palette family: four takes on warm-cream is one direction, not four. Make at least one deliberately off-distribution.
...
Build a small surface (a hero, a card, a button group) with the direction and surface it early — as
part of `factory-prototype`'s "show it live" step, or the first checkpoint of whatever build this is
inside. State plainly whether it reads as the three adjectives you committed to. If a reviewer pushes
back on an axis, revise before going broader. A direction that fails at small scale gets worse as it
scales up, not better.
