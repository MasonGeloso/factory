...
Take the aspects the task explicitly asked for; where it doesn't specify, pick the axes with the most
obvious visual variation and note the choice in your summary: color (primary, accent, background tone), typography (family, base size, scale ratio), density (tight / normal / loose), layout variant, component variants (filled/ghost buttons, card treatment), copy (headline, subhead, CTA text), feature flags (show/hide sections).
